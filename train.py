...
class TransformerLM(nn.Module):
    ...
    def forward(self, src):
        ...
        mask = torch.triu(torch.ones(seq_len_src, seq_len_src), diagonal=1).bool().to(src.device)
        output = self.transformer_encoder(src, mask=mask)
        output = self.linear(output)
        return output

model = TransformerLM(vocab_size, d_model, nhead, num_layers, dropout).to(device)
...
if model_checkpoints:
    try:
        ...
        epochs_done = [int(f.split('_')[-1].split('.')[0]) for f in model_checkpoints]
        ...
        if start_epoch > epochs:
            print(f"Model already trained up to epoch {ckpt_max_epoch}, skipping training")
        else:
...
for epoch in range(start_epoch, epochs + 1):
    ...
    for batch in tqdm(train_loader, desc=f'Epoch {epoch} Train'):
        ...
        if step % log_interval == 0:
            tqdm.write(f'Epoch {epoch}, Step {step}, Avg Loss: {total_loss / step:.4f}')
...
def generate_word(model, vocab, max_len, seq_len, device, temperature):
    ...
    generated_word = ''.join([idx_to_char[idx] for idx in generated[start_idx:end_idx]])
    return generated_word

# Generate samples
print("Generating samples...")
for i in range(10):
    gen = generate_word(model, vocab, generate_len, seq_len, device, temperature)
